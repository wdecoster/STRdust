...
OPTIONS:
    ...
    -t, --threads <THREADS>            Number of parallel threads to use [default: 1]
        ...
        --fast                         Cut the repeat sequence out of the existing alignment instead of re-aligning
        --fast-flank <FAST_FLANK>      Flanking bases considered on either side of the repeat in --fast mode [default: 10]
...
- `--fast` genotypes a locus without re-aligning anything. The default path builds an artificial reference with the repeat excised, re-aligns every spanning read to it with minimap2, and takes the sequence that fails to align as the allele. That is accurate but expensive: re-aligning whole (tens of kb) reads is essentially the entire runtime. In `--fast` mode the allele is instead cut straight out of the alignment already in the BAM/CRAM, by walking the CIGAR from the repeat start to the repeat end. The slice contains the reference-matching repeat copies as well as any inserted bases, and is shortened by deletions, so it is the full repeat sequence as the read carries it - not only the inserted part. `--fast-flank` bases on either side are included in the window and trimmed back off by query length, so indels the aligner placed just outside the annotated interval still count towards the allele. Everything downstream (haplotype splitting, consensus, VCF) is unchanged.
  - Reads that do not span the locus, or that are soft-clipped into it, cannot report an allele length and are dropped. If that leaves a haplotype below `--support`, the locus falls back to the full alignment path, so loci with very large expansions are still genotyped the sensitive way.
  - On a 30x ONT genome, `--fast` is roughly 50-100x faster per locus than the alignment path, and per-haplotype median read lengths agree with it to within 2 bp at ~98% of haplotypes.
