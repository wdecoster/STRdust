...
OPTIONS:
    ...
    -t, --threads <THREADS>            Number of parallel threads to use [default: 1]
        ...
        --fast-flank <FAST_FLANK>      Flanking bases considered on either side of the repeat with --mode fast [default: 10]
...
- `--mode` chooses how the repeat sequence of a read is recovered, trading sensitivity against speed.
  ...
  - `fast` cuts the allele straight out of the alignment already in the BAM/CRAM, by walking the CIGAR from the repeat start to the repeat end. The slice contains the reference-matching repeat copies as well as any inserted bases, and is shortened by deletions, so it is the full repeat sequence as the read carries it - not only the inserted part. `--fast-flank` bases on either side are included in the window and trimmed back off again by reference length, so indels the aligner placed just outside the annotated interval still count towards the allele; the flank is widened per read, up to 30 bases, to take in whole indels of at least 10 bases. Everything downstream (haplotype splitting, consensus, VCF) is unchanged.
  - In `fast` mode, reads that do not span the locus, or that are soft-clipped into it, cannot report an allele length and are dropped. If that leaves a haplotype below `--support`, the locus falls back to `sensitive`, so loci with very large expansions are still genotyped the sensitive way.
  - On a 30x ONT genome, `fast` genotyped 2000 catalog loci in 13 s of CPU against 1016 s for `sensitive` (77x, and 289 MB against 643 MB peak memory). Per-haplotype median read lengths (`MRL`) agree with `sensitive` to within 2 bp at 95.7% of alleles. Note that this is agreement with `sensitive`, not accuracy: neither mode has been benchmarked against a truth set yet.
